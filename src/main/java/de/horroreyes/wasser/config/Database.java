...
class Database {
    ...
    @Bean
    CommandLineRunner initDatabase(PersonRepository personRepository, PlaceRepository placeRepository, UserRepository userRepository, PasswordEncoder passwordEncoder) {
        ...
        return args -> {
            ...
            if (userRepository.count() == 0) {
                log.info(PRELOADING + userRepository.save(new User("admin", passwordEncoder.encode("admin"))));
                log.info(PRELOADING + userRepository.save(new User("user", passwordEncoder.encode("user"))));
            }
        };
    }
}
